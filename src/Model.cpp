...
Model::Model(int input_size, int layer1_size, int layer2_size, int output_size)
{
    ...
    assign_weights(int input_size, int layer1_size, int layer2_size, int output_size)
    assign_biases(int input_size, int layer1_size, int layer2_size, int output_size)
...
void Model::assign_biases(int input_size, int layer1_size, int layer2_size, int output_size)
{
    ...
    b3 = Variable(scope, {1, layer3_size}, DT_FLOAT);
    assign_b3 = Assign(scope, b3, RandomNormal(scope, {1, layer3_size}, DT_FLOAT));
}

void Model::fit(int input, int labels)
{
...
void Model::predict(int input)
{
    TF_CHECK_OK(model_session.Run({{x, {data_set.input(110000.f, Fuel::DIESEL, 7.f)}}}, {layer_3}, &outputs));
    std::cout << "DNN output: " << *outputs[0].scalar<float>().data() << endl;
    std::cout << "Price predicted " << data_set.output(*outputs[0].scalar<float>().data()) << " euros" << std::endl;
}

void Model::saveModel(int input)
{
    GraphDef graph_def;
